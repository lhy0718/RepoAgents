...
RepoRepublic는 저장소 내부 제어 파일을 설치하고, 그 위에서 이슈 기반 유지보수 실행을 조정하는 Python 오케스트레이션 프레임워크입니다.
...
src/reporepublic/
  ...
  templates/      `republic init`이 생성하는 파일
```

## 제어 평면 파일

`republic init`이 설치하는 파일:

- `.ai-republic/reporepublic.yaml`: 런타임 설정
- `.ai-republic/roles/*.md`: 역할 정의서
- `.ai-republic/prompts/*.txt.j2`: 역할 프롬프트
- `.ai-republic/policies/*.md`: 가드레일
- `AGENTS.md`: Codex가 읽는 저장소 지침
- `WORKFLOW.md`: 운영자용 파이프라인 요약

이 파일들은 런타임 계약의 일부입니다. RepoRepublic는 숨겨진 에이전트 규칙을 하드코딩하는 대신 이 파일들로 프롬프트를 렌더링합니다.

## 런타임 흐름

```mermaid
flowchart TD
    A["republic run / trigger / webhook"] --> B["설정과 상태 로딩"]
...
`republic run`은 장기 실행 polling 진입점입니다. `republic trigger <issue-id>`는 polling을 우회하고 특정 이슈 하나를 즉시 실행합니다. `republic webhook --event ... --payload ...`는 GitHub webhook payload를 파싱해 단일 이슈를 고른 뒤, 같은 single-issue 실행 경로를 재사용합니다.
...
실제 worker runtime은 RepoRepublic 밖에 있습니다. RepoRepublic는 오케스트레이션하고, Codex가 실행합니다.
...
실행 상태는 version 필드를 포함한 `.ai-republic/state/runs.json`에 저장됩니다.
...
RepoRepublic는 `.ai-republic/workspaces/issue-<id>/<run-id>/repo` 아래에서 두 가지 workspace 전략을 지원합니다.
