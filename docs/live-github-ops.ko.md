...
- RepoAgents checkout에서 `uv sync --dev`가 끝나 있음
...
RepoAgents가 유지보수할 실제 저장소에서 시작합니다.
...
`.ai-repoagents/repoagents.yaml`을 열고 아래 값들을 확인합니다.
...
- `logging.file_enabled: true`는 `.ai-repoagents/logs/repoagents.jsonl`에 운영 흔적을 남김
...
- `.ai-repoagents/artifacts/issue-123/<run-id>/` 아래 artifact
- `.ai-repoagents/workspaces/issue-123/<run-id>/repo/` 또는 worktree 경로
...
브라우저에서 `.ai-repoagents/dashboard/index.html`을 열고 다음을 활용합니다.
...
- RepoAgents가 `request_changes`를 낼 때 사람이 어떻게 판단할지 경로가 명확함
