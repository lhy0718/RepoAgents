...
RepoAgents is a Python orchestration framework that installs repository-local control files and then coordinates issue-driven maintenance runs.
...
src/repoagents/
  ...
  templates/      files generated by `repoagents init`
```

## Control plane files

`repoagents init` installs:

- `.ai-repoagents/repoagents.yaml`: runtime config
- `.ai-repoagents/roles/*.md`: role charters
- `.ai-repoagents/prompts/*.txt.j2`: role prompts
- `.ai-repoagents/policies/*.md`: guardrails
- `AGENTS.md`: Codex-readable repo instructions
- `WORKFLOW.md`: maintainer-facing flow summary

Those files are part of the runtime contract. RepoAgents renders prompts from them instead of hard-coding hidden agent behavior.

## Runtime flow

```mermaid
flowchart TD
    A["repoagents run / trigger / webhook"] --> B["Load config and state"]
...
`repoagents run` is the long-running polling entrypoint. `repoagents trigger <issue-id>` bypasses polling and runs one issue immediately. `repoagents webhook --event ... --payload ...` parses a GitHub webhook payload, selects one issue, and then reuses the same single-issue execution path.
...
Run state is persisted to a versioned `.ai-repoagents/state/runs.json`.
...
RepoAgents supports two workspace strategies under `.ai-repoagents/workspaces/issue-<id>/<run-id>/repo`:
