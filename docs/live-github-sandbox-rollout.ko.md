...
이 스크립트는 임시 저장소를 만들고, 가짜 `GITHUB_TOKEN`을 설정하고, `tracker.smoke_fixture_path`를 phase fixture로 바꿔 가며 per-phase `doctor`, `github smoke` export를 남긴 뒤, `.ai-republic/reports/ops/sandbox-pr-ready/` 아래에 readiness bundle을 만들고, 그 다음 `github fixture + mock backend`로 잠깐 전환해 issue 하나를 실행한 뒤 `.ai-republic/reports/ops/sandbox-issue-201/` 아래에 execution bundle도 생성합니다.
...
5. `uv run republic doctor`를 실행합니다.
6. `uv run republic github smoke --format all`을 실행합니다.
