...
3. Authenticate with GitHub. On a local machine, run `gh auth login`; RepoAgents will automatically reuse `gh auth token` when `GITHUB_TOKEN` is unset. In CI or other non-interactive environments, set `GITHUB_TOKEN` explicitly.
...
- `repoagents dashboard --tui` opens an interactive terminal dashboard with `Runs`, `Reports`, `Ops`, `Handoffs`, and `Retention` sections. Use `1-5`, arrow keys, `Tab`, `r`, and `q` to navigate, refresh, and exit. Press `a` on a selected row to open available actions, then use `1-9` to pick an action and `y/N` when confirmation is required.
- `repoagents dashboard` opens the terminal dashboard by default in an interactive TTY. Add `--refresh-seconds 30` to auto-reload the TUI, or use `--tui` explicitly in scripts that allocate a terminal.
- `repoagents dashboard --format all` exports JSON and Markdown snapshots together under `.ai-repoagents/dashboard/`. The Markdown snapshot mirrors the CLI related-report detail block, and the JSON export carries a ready-to-display `related_report_detail_summary` string for each report entry. When `.ai-repoagents/reports/ops-status.json` or `.ai-repoagents/reports/ops-brief.json` exists, the `Reports` section links `Ops status` and `Ops brief` into the same posture summary.
...
- `repoagents github smoke` probes live GitHub REST readiness against `tracker.repo`, samples open issues, and reports publish preflight posture for comment and draft-PR writes. In REST mode, RepoAgents first uses `GITHUB_TOKEN` and then falls back to `gh auth token`; for CI or other non-interactive environments, set `GITHUB_TOKEN` explicitly.
