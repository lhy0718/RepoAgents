...
- `repoagents dashboard --tui`는 `Runs`, `Reports`, `Ops`, `Handoffs`, `Retention` 섹션이 있는 인터랙티브 터미널 대시보드를 엽니다. `1-5`, 화살표, `Tab`, `r`, `q`로 이동, 새로고침, 종료를 할 수 있습니다. 선택된 항목에서 `a`를 누르면 가능한 작업 메뉴가 열리고, `1-9`로 작업을 고른 뒤 확인이 필요한 경우 `y/N`으로 진행할 수 있습니다.
