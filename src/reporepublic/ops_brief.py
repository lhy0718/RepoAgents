...
def _build_next_actions(
    *,
    ...
    if int(doctor_summary.get("exit_code", 0) or 0) != 0:
        actions.append("Run `republic doctor` and resolve any failing diagnostics before live publish.")
    if _normalize_severity(hero.get("severity")) in {"attention", "issues"}:
    ...
    if _normalize_optional_severity(github_smoke_summary.get("status")) in {"attention", "issues"}:
        publish_message = _string_or_none(github_smoke_publish.get("message"))
        action = "Run `republic github smoke --require-write-ready` and clear GitHub branch/publish warnings before unattended live writes."
        if publish_message:
            action = (
                "Run `republic github smoke --require-write-ready` and resolve "
