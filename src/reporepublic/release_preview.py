...
from reporepublic.utils.git import GitCommandError, is_git_repository, list_dirty_working_tree_entries, run_git
...
def build_release_preview_snapshot(
    *,
    ...
    package_init_path = resolved_repo_root / "src" / "reporepublic" / "__init__.py"
    ...
    release_title = f"RepoRepublic {resolved_target_tag}"
    ...
    *,
    ...
    if module_version is None:
        return {
            "name": "Version files",
            "status": "error",
            "message": "src/reporepublic/__init__.py does not define __version__",
    ...
    *,
    ...
    if resolved_target_version != current_version:
        return {
            ...
            "hint": "Update pyproject.toml and src/reporepublic/__init__.py before tagging the release.",
            "detail_lines": detail_lines,
        }
...
def _determine_files_to_update(
    *,
    ...
    if module_version != resolved_target_version:
        files.append("src/reporepublic/__init__.py")
    return files
...
def _build_release_commands(
    *,
    ...
    publish_commands = [
        f'git tag -a {resolved_target_tag} -m "RepoRepublic {resolved_target_tag}"',
        "git push origin main",
        f"git push origin {resolved_target_tag}",
        (
            f'gh release create {resolved_target_tag} --title "RepoRepublic {resolved_target_tag}" '
            f'--notes-file .ai-republic/reports/release-notes-{resolved_target_tag}.md'
        ),
    ]
    return {
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "- `uv run pytest -q`",
            "- `uv build`",
            "- clean wheel install smoke for `republic --help`",
            "",
            "## Operator defaults",
