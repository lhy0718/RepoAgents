...
from reporepublic.models import ExternalActionResult, IssueComment, IssueRef
from reporepublic.tracker.base import Tracker
from reporepublic.tracker.issue_loader import load_issue_file
from reporepublic.utils import ensure_dir, is_git_repository, run_git, sanitize_branch_name, sync_workspace_to_git_clone
...
class GitHubTracker(Tracker):
    def __init__(
        ...
        self.logger = get_logger("reporepublic.tracker.github")
    ...
    async def create_branch(
        ...
        run_git(
            [
                "-c",
                "user.name=RepoRepublic Bot",
                "-c",
                "user.email=republic@example.invalid",
                "commit",
                "-m",
                commit_message,
            ],
