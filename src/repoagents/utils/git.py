...
def sanitize_branch_name(raw: str, issue_id: int | None = None) -> str:
    ...
    if not candidate:
        candidate = f"repoagents/issue-{issue_id}" if issue_id is not None else "repoagents/update"
    if candidate.endswith(".lock"):
        candidate = f"{candidate}-branch"
    if not candidate.startswith("repoagents/"):
        candidate = f"repoagents/{candidate}"
