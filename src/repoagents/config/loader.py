...
DEFAULT_CONFIG_PATH = Path(".ai-repoagents/repoagents.yaml")


class ConfigLoadError(RuntimeError):
    """Raised when RepoAgents config cannot be loaded."""
...
def load_config(repo_root: Path | None = None, config_path: Path | None = None) -> LoadedConfig:
    ...
    if not path.exists():
        raise ConfigLoadError(
            f"RepoAgents config not found at {path}. Run `repoagents init` first."
        )
    try:
    ...
    try:
        config = RepoAgentsConfig.model_validate(payload)
...
def format_validation_error(path: Path, exc: ValidationError) -> str:
    lines = [f"Invalid RepoAgents config at {path}:"]
