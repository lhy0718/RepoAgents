...
from repoagents.models import CURRENT_RUN_STATE_VERSION
from repoagents.utils.files import ensure_dir, write_text_file
...
def scaffold_repository(
    ...
    ai_root = ensure_dir(repo_root / ".ai-repoagents")
...
def build_upgrade_plan(
    ...
    runs_path = repo_root / ".ai-repoagents" / "state" / "runs.json"
    if not runs_path.exists():
        plans.append(
            UpgradePlanItem(
                path=runs_path,
                label=".ai-repoagents/state/runs.json",
                action="create_state",
...
def render_managed_file_map(
    ...
    managed.update(_render_static_tree(template_root.joinpath("roles"), Path(".ai-repoagents/roles")))
    managed.update(_render_static_tree(template_root.joinpath("prompts"), Path(".ai-repoagents/prompts")))
    managed[Path(".ai-repoagents/policies/merge-policy.md")] = environment.get_template(
        "policies/merge-policy.md"
    ).render(**template_context)
    managed[Path(".ai-repoagents/policies/scope-policy.md")] = environment.get_template(
        "policies/scope-policy.md.j2"
    ).render(**template_context)
    managed[Path(".ai-repoagents/repoagents.yaml")] = environment.get_template(
        "config/repoagents.yaml.j2"
    ).render(**template_context)
    managed[Path("WORKFLOW.md")] = environment.get_template("WORKFLOW.md.j2").render(
        **template_context
    )
    managed[Path(".github/workflows/repoagents-check.yml")] = environment.get_template(
...
def _template_root() -> Path:
    return resources.files("repoagents").joinpath("templates/default")
