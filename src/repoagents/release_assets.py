...
def _artifact_version(name: str) -> str | None:
    if not name.startswith("repoagents-"):
        return None
    remainder = name[len("repoagents-") :]
...
def _run_smoke_install(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="repoagents-release-assets-") as temp_dir:
        venv_dir = Path(temp_dir) / "venv"
        command = (
            f"{sys.executable} -m venv {venv_dir} && "
            f"{venv_dir / 'bin' / 'pip'} install {wheel_path} && "
            f"{venv_dir / 'bin' / 'repoagents'} --help"
        )
        completed = subprocess.run(
            [
                "bash",
