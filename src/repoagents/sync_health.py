...
from repoagents.cleanup_report import build_cleanup_report_snapshot
from repoagents.config import LoadedConfig
from repoagents.models.domain import utc_now
from repoagents.sync_audit import build_sync_audit_snapshot
...
def _build_next_actions(
    *,
    ...
    if pending_artifacts:
        actions.append("Review pending staged artifacts with `repoagents sync ls` and `repoagents sync show`.")
    if integrity_issue_count or repair_findings_after:
        actions.append(
            "Inspect applied manifest issues with `repoagents sync check` and preview repairs with `repoagents sync repair --dry-run`."
        )
    elif repair_changed_reports:
        actions.append("Apply the reviewed manifest fixes with `repoagents sync repair`.")
    if cleanup_action_count or prunable_groups:
        actions.append("Review cleanup preview with `repoagents clean --sync-applied --dry-run --report`.")
    if related_report_mismatches or related_report_policy_drifts:
        actions.append(
            "Refresh raw sync reports with `repoagents sync audit --format all` and `repoagents clean --report --report-format all`."
        )
