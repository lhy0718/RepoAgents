...
from repoagents.release_assets import build_release_asset_exports, build_release_asset_snapshot
from repoagents.release_preview import build_release_preview_exports, build_release_preview_snapshot
...
def _build_smoke_install_check(snapshot: dict[str, Any]) -> dict[str, Any]:
    ...
    if not bool(snapshot.get("ran")):
        return {
            ...
            "hint": "Run smoke install when you want the preflight to verify `repoagents --help` from the built wheel.",
            "detail_lines": detail_lines,
        }
    if snapshot.get("status") == "error":
        return {
