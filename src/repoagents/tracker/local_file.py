...
from repoagents.models import ExternalActionResult, IssueRef
from repoagents.tracker.base import Tracker
from repoagents.tracker.issue_loader import load_issue_file
from repoagents.utils.files import ensure_dir, write_json_file, write_text_file
...
class LocalFileTracker(Tracker):
    def __init__(self, path: Path, repo_root: Path, dry_run: bool) -> None:
        ...
        self.sync_root = repo_root / ".ai-repoagents" / "sync" / "local-file"

    async def list_open_issues(self) -> list[IssueRef]:
        return load_issue_file(self.path)

    async def get_issue(self, issue_id: int) -> IssueRef:
        for issue in load_issue_file(self.path):
            if issue.id == issue_id:
                return issue
        raise KeyError(f"Issue {issue_id} not found in local tracker file {self.path}.")
    ...
    async def post_comment(self, issue_id: int, body: str) -> ExternalActionResult:
        if self.dry_run:
            return ExternalActionResult(
                action="post_comment",
                executed=False,
                reason="Dry-run blocks local_file sync staging.",
        ...
        stage_path = self._stage_markdown(
            ...
            "comment",
            body,
            metadata={"issue_id": issue_id, "action": "post_comment"},
        )
        return ExternalActionResult(
            action="post_comment",
            executed=True,
            reason="Staged issue comment in local_file sync directory.",
    ...
    async def create_branch(
        ...
        if self.dry_run:
            return ExternalActionResult(
                ...
                executed=False,
                reason="Dry-run blocks local_file branch proposal staging.",
        ...
        return ExternalActionResult(
            action="create_branch",
            executed=True,
            reason="Staged branch proposal in local_file sync directory.",
            payload={
                "issue_id": issue_id,
                "path": str(self.path),
                "requested_name": name,
    ...
    async def open_pr(
        ...
        if self.dry_run:
            return ExternalActionResult(
                action="open_pr",
                executed=False,
                reason="Dry-run blocks local_file draft PR staging.",
                payload={"issue_id": issue_id, "path": str(self.path)},
            )
        ...
        return ExternalActionResult(
            action="open_pr",
            executed=True,
            reason="Staged draft PR proposal in local_file sync directory.",
            payload={
                "issue_id": issue_id,
                "path": str(self.path),
    ...
    async def set_issue_label(self, issue_id: int, labels: list[str]) -> ExternalActionResult:
        if self.dry_run:
            return ExternalActionResult(
                ...
                executed=False,
                reason="Dry-run blocks local_file label staging.",
        ...
        return ExternalActionResult(
            action="set_issue_label",
            executed=True,
            reason="Staged label proposal in local_file sync directory.",
    ...
    def _stage_json(self, issue_id: int, action: str, payload: dict[str, object]) -> Path:
        ...
        write_json_file(target, {"action": action, "staged_at": stamp, **payload})
        return target
    ...
    def _stage_markdown(
        ...
        *,
        ...
        frontmatter_body = yaml.safe_dump(frontmatter, sort_keys=False).strip()
        write_text_file(target, f"---\n{frontmatter_body}\n---\n\n{body.rstrip()}\n")
        return target
