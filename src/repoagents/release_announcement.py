...
from repoagents.release_preview import build_release_preview_snapshot
...
def build_release_announcement_snapshot(
    *,
    ...
    release_title = str(target.get("title") or f"RepoAgents {target.get('tag', '')}")
...
def _build_announcement_copy(
    *,
    ...
    lines = [
        f"# {title}",
        "",
        f"RepoAgents {tag} is now available as a public preview.",
        "",
        "RepoAgents installs an AI maintainer team into any repo, with Codex CLI as the default live worker runtime and human approval as the default safety boundary.",
        "",
        "## Highlights",
    ]
    lines.extend(f"- {item}" for item in highlights)
    lines.extend(
        [
            "",
            "## Start here",
            "- README: https://github.com/lhy0718/RepoAgents",
            "- Quickstart: https://github.com/lhy0718/RepoAgents/blob/main/QUICKSTART.md",
            "- Release checklist: https://github.com/lhy0718/RepoAgents/blob/main/docs/release.md",
            "",
            f"Tag: `{tag}`",
            f"Version: `{version}`",
        ]
...
def _build_discussion_copy(
    *,
    ...
    lines = [
        ...
        "",
        f"We have cut `{tag}` as the first public preview of RepoAgents.",
        "",
...
def _build_social_copy(
    *,
    ...
    return "\n".join(
        [
            f"RepoAgents {tag} is out in public preview.",
            "Install an AI maintainer team into any repo.",
            first_highlight,
            second_highlight,
            "GitHub: https://github.com/lhy0718/RepoAgents",
        ]
    )
...
def _extract_highlights(unreleased_body: str) -> list[str]:
    ...
    if not highlights:
        highlights.append("RepoAgents public preview release.")
    return highlights[:4]
