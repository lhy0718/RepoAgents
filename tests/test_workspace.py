...
from reporepublic.utils import run_git
from reporepublic.workspace import WorkspaceError, WorktreeWorkspaceManager, build_workspace_manager
...
def test_workspace_copy_isolated(demo_repo: Path) -> None:
    ...
    assert (workspace / ".ai-republic" / "prompts" / "triage.txt.j2").exists()
    assert not (workspace / ".ai-republic" / "state").exists()
