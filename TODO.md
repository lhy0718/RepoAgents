...
1. `ops snapshot` 상태를 dedicated `republic ops status` surface로 분리하기
  - [x] `republic ops status` subcommand 추가
  - [x] latest/history index와 최신 `bundle.json`을 함께 읽는 snapshot builder 추가
  - [x] text + JSON/Markdown export 추가
  - [x] 테스트와 문서 갱신
