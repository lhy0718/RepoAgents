...
- [x] `ops snapshot` handoff summary를 operator-facing incident brief로 강화하기
...
- [x] `ops brief`를 dashboard/report flow와 handoff archive landing page에 노출하기
  - dashboard `Reports`가 `Ops brief` export를 카드로 렌더링하고 `Ops snapshots`가 landing link와 current brief headline/severity를 함께 표시함
...
- [x] live GitHub integration test 범위를 publish path까지 넓히기
  ...
  - smoke readiness가 repo metadata `permissions.push`도 반영하도록 보강
  - runbook과 safety guardrail 문서 갱신

## 추천 다음 순서

1. [x] `github smoke`/`github-smoke.*`를 `ops snapshot` bundle과 dashboard/report flow에 포함하기
  - live GitHub repo readiness를 incident handoff bundle에서 바로 공유할 수 있게 `ops snapshot` optional component로 묶기
  - dashboard `Reports`와 `ops status`가 latest `github-smoke` posture를 card/related report로 같이 읽게 연결
  - live GitHub 운영 runbook을 handoff bundle 기준으로 다시 정리

2. [x] `ops snapshot`과 `github smoke`를 결합한 live rollout/handoff 예제를 추가하기
  - live GitHub REST tracker에서 `github-smoke`, `ops-brief`, `ops-status`, landing page를 함께 생성하는 end-to-end 예제를 보강
  - incident handoff archive 기준으로 operator가 어떤 파일을 먼저 열어야 하는지 예제/문서에 고정

3. publish-enabled sandbox rollout 예제를 추가하기
  - `allow_write_comments` 또는 `allow_open_pr`를 sandbox repo 기준으로 점진 활성화하는 예제를 별도 문서/스크립트로 제공
  - live publish readiness와 rollout gate를 예제 수준에서 다시 보여주기
