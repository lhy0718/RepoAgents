...
RepoAgents MVP 이후 구현할 우선순위 backlog입니다. 현재 코드는 로컬 데모와 mock backend 기준으로는 동작하지만, 실운영 완성도를 높이려면 아래 항목이 필요합니다.
...
- [x] structured logging 파일 출력 지원
  - `logging.file_enabled: true`일 때 `.ai-repoagents/logs/repoagents.jsonl`에 JSONL 로그를 남김
...
- [x] `repoagents init` 대화형 모드 추가
  - 플래그 없이 실행하면 preset, tracker repo, backend mode, fixture path를 순서대로 안내함
  - `--backend`와 비대화형 플래그 경로도 계속 지원함

- [x] `repoagents doctor` 진단 항목 확장
  - GitHub auth, network reachability, template drift, write 권한 점검이 추가됨
  - 실패 원인과 해결 힌트를 더 구체적으로 출력함

- [x] `repoagents init --upgrade` 또는 템플릿 drift 감지
  - `repoagents init --upgrade`는 drift를 보여주고 missing managed 파일을 복구함
  - `repoagents init --upgrade --force`는 drifted managed 파일까지 refresh함
...
- [x] webhook receiver server 예제 추가
  - `scripts/webhook_receiver.py`로 GitHub 스타일 POST를 `repoagents webhook`으로 포워딩하는 로컬 HTTP receiver 예제를 추가함
...
- [x] dashboard export/share formats 추가
  - `repoagents dashboard --format html|json|markdown|all` 경로를 추가해 HTML 외 export를 지원함
...
- [x] additional custom tracker write paths or sync adapters 추가
  - `local_markdown` tracker가 `.ai-repoagents/sync/local-markdown/issue-<id>/` 아래에 comment, branch, label, draft PR proposal을 stage할 수 있게 함
...
- [x] tracker sync artifact export/apply utility 추가
  - `repoagents sync ls`와 `repoagents sync show`로 staged sync artifact inventory를 조회할 수 있게 함
  - 공통 sync contract를 `docs/sync.md`, `docs/sync.ko.md`에 문서화함

- [x] tracker-specific sync apply helpers 추가
  - `repoagents sync apply`로 `local_markdown` comment/labels artifact를 source issue에 반영하고 handled artifact를 `.ai-repoagents/sync-applied/`로 archive함
  - `sync ls --scope applied`와 demo/runbook 문서로 apply 이후 lifecycle을 확인할 수 있게 함

- [x] `branch`/`pr-body` artifact bundle apply helper 추가
  - `repoagents sync apply --bundle`로 관련 `branch`, `pr`, `pr-body` handoff set을 한 번에 archive할 수 있음
...
- [x] scaffold snapshot 테스트 추가
  - `repoagents init`이 생성하는 파일이 의도치 않게 바뀌지 않도록 해야 함
...
- [x] webhook 또는 event-driven 실행 모드
  - `repoagents trigger <issue-id>`로 polling 없이 단일 issue run/dry-run을 시작할 수 있음
  - `repoagents webhook --event issues --payload payload.json`으로 GitHub webhook payload를 받아 단일 issue run을 시작할 수 있음

- [x] 장기적으로 웹 UI 또는 운영 대시보드
  - `repoagents dashboard`가 현재 run 상태, artifact 링크, 실패 사유를 정적 HTML로 생성함
  - 기본 출력은 `.ai-repoagents/dashboard/index.html`

- [x] dashboard filtering/search와 live refresh 추가
  - 대시보드에 client-side 검색, 상태 필터, 빈 결과 메시지를 추가함
  - `repoagents dashboard --refresh-seconds <n>`으로 timed reload 기본값을 심을 수 있음

- [x] dashboard/export에 normalized sync metadata 링크 연결
  - dashboard가 `.ai-repoagents/sync-applied/**/manifest.json`을 읽어 `Sync handoffs` 섹션을 함께 렌더링함
...
- [x] sync artifact cleanup/retention 정책을 manifest-aware하게 정리
  - `repoagents clean --sync-applied`가 `handoff.group_key` 단위 retention을 적용함
  - orphan archive 파일과 dangling manifest entry도 함께 보수적으로 정리함

- [x] applied sync manifest integrity 검사와 repair helper 추가
  - `repoagents sync check`가 duplicate key, dangling archive, orphan file, handoff mismatch를 보고함
  - `repoagents sync repair`가 canonicalize, orphan adoption, handoff linkage 재구성을 수행함

- [x] sync-applied retention 결과를 dashboard에 age/size 기준으로 시각화
  - `repoagents dashboard`가 `Sync retention` 섹션에서 issue별 `stable/prunable/repair-needed` 상태를 노출함
  - JSON/Markdown export도 prunable group 수, bytes, oldest prunable age를 함께 남김

- [x] sync audit report export 추가
  - `repoagents sync audit --format all`이 `.ai-repoagents/reports/` 아래에 JSON/Markdown report를 생성함
...
- [x] sync-applied cleanup 결과를 machine-readable report로 남기기
  - `repoagents clean --sync-applied --dry-run --report`가 cleanup preview/result를 `.ai-repoagents/reports/` 아래에 저장함
  - report가 action list, affected issue, manifest rewrite count를 남김

- [x] sync audit / cleanup report를 dashboard에서 바로 열 수 있게 연결
  - `repoagents dashboard`가 `.ai-repoagents/reports/`를 읽어 `Reports` 섹션과 HTML/JSON/Markdown report summary를 함께 렌더링함
  - sync audit/cleanup export가 있을 때 dashboard에서 직접 링크와 상태 요약을 확인할 수 있음

- [x] cleanup report를 sync audit에 cross-link
  - `repoagents sync audit` snapshot이 matching `cleanup-preview` / `cleanup-result` export를 읽어 함께 요약함
...
- [x] sync audit related cleanup report에 issue filter mismatch 경고 추가
  - `repoagents sync audit --issue <id>`가 다른 `issue_filter`로 생성된 cleanup report를 mismatch warning으로 분리해 보여줌
...
- [x] doctor에 report freshness policy 진단을 추가
  - `repoagents doctor`가 현재 threshold를 그대로 출력함
  - stale/unknown escalation이 과도하게 완화된 경우 WARN과 hint를 함께 보여줌

- [x] report freshness severity를 `repoagents status` export와 연결
  - `repoagents status`가 dashboard와 같은 report-health snapshot을 읽어 severity를 함께 출력함
...
- [x] status 출력에도 policy threshold summary를 함께 붙이기
  - `repoagents status`가 report health summary 바로 아래에 current policy summary를 출력함
...
- [x] sync audit/cleanup related report policy drift를 CLI summary로 노출하기
  - `repoagents sync audit`가 linked cleanup policy drift count를 summary에 출력함
  - `repoagents clean --report`가 linked sync-audit policy drift count를 summary에 출력함
...
- [x] report policy drift를 doctor/status policy health summary와 통합
  - `repoagents doctor`가 threshold relaxation과 embedded-policy drift를 합친 `Report policy health` summary를 출력함
  - `repoagents status`도 active threshold와 mismatch warning 옆에 같은 `policy_health` 요약을 출력함
...
- [x] report policy drift remediation guidance를 doctor/status/dashboard에서 공통 helper로 정리
  ...
  - dashboard card, `repoagents doctor`, `repoagents status`가 같은 re-export guidance를 재사용함
...
- [x] sync audit/cleanup CLI summary에도 remediation guidance를 opt-in으로 직접 출력
  - `repoagents sync audit --show-remediation`과 `repoagents clean --report --show-remediation`이 drift가 있을 때 guidance를 출력함
  - 기본 summary 출력은 기존처럼 count 중심으로 유지됨

- [x] sync audit/cleanup CLI summary에 mismatch detail도 opt-in으로 직접 출력
  - `repoagents sync audit --show-mismatches`와 `repoagents clean --report --show-mismatches`가 linked report issue-filter mismatch warning을 직접 출력함
...
- [x] related-report internal module을 package로 분리
  ...
  - import 표면은 `repoagents._related_report_details`에서 유지하고 내부 구조만 정리
...
- [x] `doctor/status` operator snapshot export 추가
  - `--format json|markdown|all`, `--output`을 지원하고 `.ai-repoagents/reports/doctor.*`, `status.*` 기본 export 경로를 제공
...
- [x] `ops snapshot bundle` latest pointer/history index 추가
  - 매 `repoagents ops snapshot` 실행이 `.ai-repoagents/reports/ops/latest.*`, `history.*`를 갱신함
  - latest entry가 bundle/archive path와 component status를 함께 기록함

- [x] `ops snapshot bundle` history retention/prune policy 연결
  - `cleanup.ops_snapshot_keep_entries`와 `cleanup.ops_snapshot_prune_managed`를 `repoagents ops snapshot` 실행 경로에 연결
  - `--history-limit`, `--prune-history`로 실행 단위 override를 지원
  - dropped history entry 중 `.ai-repoagents/reports/ops/` 아래 managed bundle/archive만 안전하게 정리

- [x] `ops snapshot` history/index 상태를 dashboard와 status surface에 노출
  - dashboard가 `.ai-repoagents/reports/ops/latest.*`, `history.*`를 읽어 `Ops snapshots` section과 metric을 렌더링함
  - `repoagents status`와 `status.json|md` export가 같은 ops snapshot summary를 포함함

## 최근 완료된 큰 작업

- [x] live GitHub 운영 경로 guardrail과 smoke path 강화
  - `doctor`가 GitHub repo access와 live publish readiness를 별도 진단함
  - `repoagents github smoke`가 live REST readiness, issue sampling, publish preflight를 출력/export함
  - REST mode에서는 `GITHUB_TOKEN`이 실제 요구사항이고 `gh auth`만으로는 충분하지 않다는 guardrail 반영

- [x] `sync check / repair / audit / clean`을 하나의 운영 흐름으로 더 강하게 묶기
  - `repoagents sync health`가 pending staged artifact, applied manifest integrity, repair preview, cleanup preview, related raw report posture를 하나의 snapshot으로 묶음
...
- [x] `sync health`를 `ops snapshot` bundle과 dashboard/report flow에 포함하기
  - `repoagents ops snapshot`이 bundle-local `sync-health.json|md`와 root `.ai-repoagents/reports/sync-health.json|md`를 함께 갱신함
  - bundle manifest와 `ops status`가 `sync_health` component / related report link를 함께 기록함

- [x] `ops snapshot` handoff summary를 operator-facing incident brief로 강화하고 landing surface에 연결하기
  - bundle-local `ops-brief.json|md`와 root `.ai-repoagents/reports/ops-brief.json|md` export 추가
  - `ops snapshot` bundle이 `index.html`, `README.md` landing artifact를 생성하고 `ops status`가 같은 landing/brief path를 surface에 노출함

- [x] 오픈소스 공개 준비 묶음 추가
  - `LICENSE`, `CONTRIBUTING.md`, `SECURITY.md`, `CODE_OF_CONDUCT.md`, `CHANGELOG.md` 추가
  - `docs/release.md`, `docs/release.ko.md`로 release checklist 문서화
  - GitHub Actions CI에서 `uv sync --dev`, `uv run pytest -q`, `uv build`, wheel smoke install 수행

- [x] live GitHub integration test 범위를 publish path까지 넓히기
  - opt-in live test를 read-only / comment write / draft PR publish로 분리
  - comment write test는 cleanup 시 comment를 삭제하고, draft PR test는 PR close + branch delete까지 수행

- [x] live GitHub handoff/runbook 흐름 정리
  - `github smoke`를 `ops snapshot` bundle과 dashboard/report flow에 포함
  - live rollout/handoff 예제와 publish-enabled sandbox rollout 예제를 추가
  - sandbox publish rehearsal을 실제 single-issue execution 예제와 연결

- [x] 공개 프리뷰 릴리스 rehearsal surface 정리
  - `repoagents release preview`, `repoagents release announce`, `repoagents release assets` 추가
  - local tag rehearsal, publish dry-run 예제, release asset 검증 경로를 runnable하게 제공

- [x] 오픈소스 배포 전 release preflight checklist 정리
  - `repoagents release check --format all`이 preview/announce/assets/test/build/smoke/governance gate를 한 번에 수행
  - `scripts/release_preflight.sh`로 실제 배포 직전 한 줄 실행 경로 제공
