...
- [x] live GitHub 운영 경로 guardrail과 smoke path 강화
  ...
  - `republic github smoke`가 live REST readiness, issue sampling, publish preflight를 출력/export함
...
- [x] 공개 프리뷰 릴리스 rehearsal surface 정리
  - `republic release preview`, `republic release announce`, `republic release assets` 추가
  - local tag rehearsal, publish dry-run 예제, release asset 검증 경로를 runnable하게 제공

- [x] 오픈소스 배포 전 release preflight checklist 정리
  - `republic release check --format all`이 preview/announce/assets/test/build/smoke/governance gate를 한 번에 수행
